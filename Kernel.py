...
from SimAnswer import SimilarAnswer
...
    如果filename包含通配符，则所有匹配的文件都将被加载并学习。         """
    ...
    def respond(self, input_, sessionID = _globalSessionID):
        ...
        try:
            ...
            finalResponse = u""
            for s in sentences:
                # 在获取响应之前将输入添加到历史记录列表中，以便<input />标记正常工作。
                ...
                response = self._respond(s, sessionID)  # 获取响应
    ...
    def _respond(self, input_, sessionID):
        ...
        if elem is None:
            if self._verboseMode:           #当所有模板无法匹配时，提示错误。
                self.Sim.get_answer(subbedInput)  #对替换后的输入，使用句子相似度策略作出响应。
...
    如果<condition>元素只有一个'name'属性，那么它的内容是一系列<li>元素，每个元素都有一个'value'属性。
    ...
    def _processThatstar(self, elem, sessionID):
        ...
        <thatstar> 元素类似于 <star> 元素, 除了 where <star/> returns the portion of the input string
